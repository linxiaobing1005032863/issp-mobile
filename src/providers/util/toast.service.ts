  ...
  constructor(
   ...
   get(url:any,option?:any) {
      let headers = this.setHeader();
       return this.http.get(url + this.toQueryString(option),new RequestOptions({headers: headers})).timeout(10000)
        .toPromise()
   ...
   public post(url: string, paramObj?: any) {
     let headers = this.setHeader();
     return this.http.post(url,  this.toBodyString(paramObj), new RequestOptions({headers: headers})).timeout(15000)
       .toPromise()
       .then(res => this.handleSuccess(res.json()))
       .catch(error => this.handleError(error));
   }
   public put(url: string, paramObj?: any) {
     let headers = this.setHeader();
     return this.http.put(url,this.toBodyString(paramObj), new RequestOptions({headers: headers})).timeout(15000)
       .toPromise()
       .then(res => this.handleSuccess(res.json()))
       .catch(error => this.handleError(error));
   }
